...
export default function RoomTypeDetails() {
  ...
  useEffect(() => {
    const loadRooms = async () => {
      try {
        ...
        // فلترة الغرف حسب الاسم (النوع)
        const filtered = allRooms.filter(r => r.name === decodedTypeName && r.status !== "مقفلة")
        setRooms(filtered)
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const availableRoom = rooms.find(r => r.status === "متاح")
      ...
      await addDoc(collection(db, "bookings"), {
        ...form,
        unitId: availableRoom.id,
        unitNumber: availableRoom.unitNumber || "",
    ...
    return (
      <div className="min-h-screen flex flex-col bg-[#FAF8F3]">
        ...
        <div className="flex-1 flex items-center justify-center text-center p-4">
          <div>
            <p className="text-xl text-[#7C7469] mb-4">لم يتم العثور على غرف من هذا النوع</p>
            <Link to="/rooms" className="text-[#C6A76D] hover:underline">
              ← العودة للغرف
            </Link>
          </div>
        </div>
  ...
  const availableCount = rooms.filter(r => r.status === "متاح").length
  ...
  return (
    <div dir="rtl" className="min-h-screen flex flex-col bg-[#FAF8F3]">
      ...
      <div className="flex-1 max-w-4xl mx-auto py-10 text-right px-4">
        ...
        >
          {displayRoom.images?.map((img: string, i: number) => (
            <SwiperSlide key={i}>
              <img 
                src={img} 
        ...
        <div className="mt-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-[#7C7469] mt-1">🛏️ {rooms.length} وحدات من هذا النوع</p>
          </div>
          <div className="text-left">
            <p className="text-2xl font-bold text-green-600">{displayRoom.price} ريال</p>
        ...
        <p className="mt-6 text-gray-600 leading-relaxed">
          {displayRoom.description || "استمتع بإقامة مميزة في غرفنا الفاخرة المصممة بعناية لتوفر لك الراحة والرفاهية."}
        </p>
          ...
          <div className="mt-6 bg-white border border-[#E8E1D6] rounded-2xl p-6 shadow-sm">
            ...
            <div className="bg-[#FAF8F3] border border-[#E8E1D6] rounded-xl p-4 mb-4">
              <div className="flex justify-between items-center">
                <span className="text-[#7C7469]">نوع الغرفة:</span>
                <span className="font-bold text-[#2B2A28]">{decodedTypeName}</span>
              </div>
              <div className="flex justify-between items-center mt-2">
                <span className="text-[#7C7469]">السعر/ليلة:</span>
                <span className="font-bold text-green-600">{displayRoom.price} ريال</span>
              </div>
